...
# This script downloads VGA videos for a specific sequence:
# ./download_cams_video.sh [calibDir] [sequenceName] [outputDir] --snu-endpoint (optional)
#
# e.g.:
# ./download_cams_video.sh ./calib/ sampleData /data/videos/
#

calibDir=${1-""}
datasetName=${2-sampleData}
outputDir=${3-"$datasetName"}
...
# Build set of allowed VGA cameras from calibration files
# Files follow the format: calib_vga_panelId_camId.json
declare -A allowedVGA
if [[ -n "$calibDir" && -d "$calibDir" ]]; then
    for f in "$calibDir"/calib_vga_*.json; do
        base=$(basename "$f" .json)
        panelId=$(echo "$base" | cut -d_ -f3)
        camId=$(echo "$base"   | cut -d_ -f4)
        key=$(printf "%02d_%02d" "$((10#$panelId))" "$((10#$camId))")
        allowedVGA["$key"]=1
    done
    echo "Found ${#allowedVGA[@]} VGA cameras in $calibDir"
else
    echo "Warning: calibDir '$calibDir' not found or not specified. Skipping VGA downloads."
    exit 1
fi

# Helper: check directory, warn if non-empty and ask user
check_or_create_dir() {
    local dir="$1"
    if [[ -d "$dir" ]]; then
        if [[ -n "$(ls -A "$dir" 2>/dev/null)" ]]; then
        echo "Warning: directory '$dir' already exists and is not empty."
        read -rp "Do you want to empty it and continue? [y/N] " answer
        case "$answer" in
            [yY]|[yY][eE][sS])
            echo "Emptying '$dir'..."
            rm -rf "${dir:?}"/*
        ...
        # directory exists and is empty: proceed normally
    else
        mkdir -p "$dir"
    fi
}

# Output directory: videos saved directly here
check_or_create_dir "$outputDir"

######################
# Download vga videos
######################

# Videos are saved as: {outputDir}/{sequenceName}_vga_{panelId}_{camId}.mp4
...
# Track already-downloaded cameras to avoid duplicates
declare -A downloadedVGA

for (( c=0; c<$totalVGA; c++ )); do
    panelId=${panels[c]}
    camId=${nodes[c]}
    key=$(printf "%02d_%02d" "$((10#$panelId))" "$((10#$camId))")

    # Skip if not in the allowed set
    if [[ -z "${allowedVGA[$key]}" ]]; then
        continue
    fi

    # Skip duplicates (same camera can appear multiple times in the list)
    if [[ -n "${downloadedVGA[$key]}" ]]; then
        continue
    fi
    downloadedVGA["$key"]=1

    # Name includes sequence name, vga label, panel and cam ids
    fileName=$(printf "${datasetName}_vga_%02d_%02d.mp4" "$((10#$panelId))" "$((10#$camId))")
    echo "Downloading: $fileName"
    cmd=$(printf "$WGET $mO $outputDir/$fileName $endpoint/webdata/dataset/$datasetName/videos/vga_shared_crf10/vga_%02d_%02d.mp4 || rm -v $outputDir/$fileName" "$((10#$panelId))" "$((10#$camId))")
    eval "$cmd"
done